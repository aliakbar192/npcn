...
const USPSection = () => {
  ...
  return (
    <section className="py-16 md:py-24 bg-gray-900 shadow-xl shadow-gray-900/50">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {usp.map((item, index) => (
            <motion.div
              key={item.id}
              className="bg-gray-900 rounded-xl shadow-md p-6 flex border border-gray-700"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              <div className="mr-4 bg-green-900/30 rounded-full p-3 self-start">
                {getIcon(item.icon)}
              </div>
              <div>
